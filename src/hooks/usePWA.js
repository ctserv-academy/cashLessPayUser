...
export function usePWA() {
    ...
    useEffect(() => {
        ...
        const handleInstall = (event) => {
            ...
            // Show a modal or toast message instead of an alert
            ...
            `;
            message.innerHTML = `
                <h3 style="margin: 0 0 10px 0;">App Installed Successfully! 🎉</h3>
                <p style="margin: 0;">Please open the app from your home screen for the best experience.</p>
            `;
    ...
    useEffect(() => {
        const checkServiceWorkerVersion = () => {
        ...
        // Listen for version info from service worker
        const handleVersionInfo = (event) => {
            if (event.data.type === 'VERSION_INFO') {
                console.log('Service Worker version:', event.data.cacheVersion);
                console.log('App version:', event.data.appVersion);

                // You can compare versions here and force reload if needed
                const currentVersion = process.env.REACT_APP_VERSION || '1.0.0';
                if (event.data.appVersion !== currentVersion) {
        ...
        const versionCheckInterval = setInterval(checkServiceWorkerVersion, 1000 * 60 * 60); // Check every hour
